package shitengine.backend.scalemodes;
...
class FullScreenScalemode extends flixel.system.scaleModes.BaseScaleMode
{
	...
	/**
	 * Singleton instance of the `FullScreenFullScreenScalemodeScaleMode`.
	 */
	...
	public function onMeasureAwait(Width:Int, Height:Int):Void
	{
		...
		#if mobile
		updateDeviceNotch(backend.scalemodes.ScreenUtilAndroid.getNotchRect());
		#end
		updateScaleOffset();
		updateGamePosition();
		...
		for (i => bitmap in cutoutBitmaps)
		{
			if (bitmap == null)
			{
				...
				cutoutBitmaps[i] = bitmap = new Bitmap(new BitmapData((ratioAxis == X ? Math.ceil(cutoutSize.x / 2) : Math.ceil(FlxG.scaleMode.gameSize.x))
					+ 1,
					(ratioAxis == Y ? Math.ceil(cutoutSize.y / 2) : Math.ceil(FlxG.scaleMode.gameSize.y))
					+ 1, true, 0xFF000000));
	...
	public static function removeCutouts(tweenDuration:Float = 0.0, ?ease:Float->Float):Void
	{
		for (i => bitmap in cutoutBitmaps)
		{
			if (bitmap == null)
			{
				FlxG.log.warn(" WARNING " + " Tried to remove a cutout bar but there don't seem to be any.");
				continue;
			}
