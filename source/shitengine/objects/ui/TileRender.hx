 ...
 * ## Basic Usage
 * ```haxe
 * var tailSprite:TileRender = new TileRender(100, 100).loadGraphic("assets/spritesheet.png", true, 50, 50);
...
class TileRender extends FlxSprite
{
	 ...
	 */
	public function setTailAnim(name:String):TileRender
	{
		tailAnim = name;
		return this;
	}

	@:noCompletion inline function adjustFrame(frame:FlxFrame):Void
	{ // Frame gap fix made by RapperGF.
		if (frame == null || frame.frame.height < 17)
			return;
	...
	{
		if (frames == null || animation == null || tailAnim == null || animation.getByName(tailAnim) == null)
			return;
	...
	@:noCompletion override function drawComplex(camera:FlxCamera):Void
	{
		...
		final batch:FlxDrawQuadsItem = camera.startQuadBatch(_frame.parent, colorTransform?.hasRGBMultipliers(), colorTransform?.hasRGBAOffsets(), blend,
			antialiasing, shader);
