...
import shitengine.backend.input.Controls;
#if android import extension.androidtools.*; #end
import shitengine.backend.modding.PolymodHandler;
import shitengine.backend.settings.SaveData;
import shitengine.states.menus.TitleState;
import shitengine.states.options.subs.ControlsSubstate;
...
class InitState extends flixel.addons.transition.FlxTransitionableState
{
	...
	{
		FlxG.scaleMode = new shitengine.backend.scalemodes.FullScreenScalemode();
		#if android
		...
		Adapter.instance = new shitengine.backend.graphics.ModchartBackend();
