				...
				case DClass(c1):
					...
					if (COMBINE_METAS.contains(metaName))
					{
						...
						for (fld in c1.fields)
						{
							...
							var fldMetaName:Null<String> = fldMetaNames.find(n -> INSERT_METAS.contains(n) || OVERRIDE_METAS.contains(n)
								|| ADD_METAS.contains(n));
								...
								switch (fld.kind)
								{
									case KFunction(f1):
										switch (baseClass.fields[baseFieldIndex].kind)
										{
											...
											default:
												Polymod.warning(ASSET_MERGE_FAILED,
													'Field ${fld.name} from the base class ${baseClass.name} is not a function. Skipping.');
										}
							...
							else if (ADD_METAS.contains(fldMetaName))
							{
								...
								{
									Polymod.warning(ASSET_MERGE_FAILED,
										'Field ${fld.name} from the merge class ${c1.name} already exists in the base class. Skipping.');
								}
								else
								{
									baseClass.fields.push(fld);
							...
							Polymod.warning(ASSET_MERGE_FAILED,
								'Field ${fld.name} from the merge class ${c1.name} doesn\'t have any merge metadata. Skipping.');
						}
					}
